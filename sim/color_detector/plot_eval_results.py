    ...
    with open(filepath, 'r') as f:
        ...
        for row in reader:
            try:
                ...
                error_dx = float(row['error_dx_mm']) if row.get('error_dx_mm') else 0
                error_dy = float(row['error_dy_mm']) if row.get('error_dy_mm') else 0
                error_dz = float(row['error_dz_mm']) if row.get('error_dz_mm') else 0
...
def plot_position_error_histogram(gt_results: Dict[str, GTTargetResult], output_dir: str):
    ...
    # Don't add a legend here (stats box includes mean/median values)
    ax.grid(True, alpha=0.3)
    
    stats_text = (f'N = {len(errors)} targets (excl. {outliers_count} outliers >2mm)\n'
...
def plot_pointing_time_distribution(gt_results: Dict[str, GTTargetResult], output_dir: str):
    ...
    # Filter out outliers: pointing times > 2s
    successful_filtered = [r for r in successful if r.total_time_spent <= 8.0]
    outliers_count = len(successful) - len(successful_filtered)
    
    if not successful_filtered:
        print(f"No targets after outlier removal for timing plot (excluded {outliers_count} outliers >8s)")
        return
    ...
    ax1.set_ylabel('Number of GT Targets', fontsize=12)
    ax1.set_title(f'Pointing Time Distribution\n(Excluded {outliers_count} outliers >8s)', fontsize=14)
    ax1.legend()
