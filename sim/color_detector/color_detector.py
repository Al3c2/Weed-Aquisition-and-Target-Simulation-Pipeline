...
class TrackedTarget:
    ...
    needs_final_retry_movement: bool = False  # move before final retry attempt
...
class BallDetectorV419(Node):
    def __init__(self):
        ...
        self.declare_parameter('association_radius', 0.03)
        ...
        self.declare_parameter('trigger_min_x', -0.2)
        self.declare_parameter('trigger_max_x', 0.2)
        ...
        self.declare_parameter('swincar_cmd_vel_topic', '/swincar/cmd_vel')
        self.declare_parameter('final_retry_movement_x', 0.05)
        self.declare_parameter('final_retry_movement_timeout', 3.0)
        ...
        self.movement_start_y = None  # track y position before final retry movement
    ...
    def _load_parameters(self):
        ...
        self.swincar_cmd_vel_topic = self.get_parameter('swincar_cmd_vel_topic').value
        self.final_retry_movement_x = self.get_parameter('final_retry_movement_x').value
        self.final_retry_movement_timeout = self.get_parameter('final_retry_movement_timeout').value
    ...
    def _setup_publishers(self):
        ...
        self.cmd_vel_pub = self.create_publisher(
            Twist, self.swincar_cmd_vel_topic, 10)
        self.debug_pub = self.create_publisher(Image, '/ball_detector/debug_image', 10)

    def _print_config(self):
        self.get_logger().info("=" * 60)
        self.get_logger().info("Ball Detector v4.20 - Singularity avoidance via repositioning")
        self.get_logger().info("=" * 60)
        ...
        self.get_logger().info(f"Final retry movement: {self.final_retry_movement_x}m in +x direction")
        self.get_logger().info("=" * 60)
    ...
    def state_machine_callback(self):
        ...
        elif self.state == "MOVING_FOR_FINAL_RETRY":
            if self.state_start_time is not None:
                elapsed = (now - self.state_start_time).nanoseconds / 1e9
                # Check if movement has been completed (swincar has moved and stopped)
                if self.pending_target is not None and self.movement_start_y is not None:
                    current_y = self.robot_y
                    movement_distance = abs(current_y - self.movement_start_y)
                    # Wait until movement is done and swincar is stopped
                    if movement_distance >= self.final_retry_movement_x and self.swincar_stopped:
                        self.get_logger().info(f"✅ Movement complete ({movement_distance:.4f}m) - retrying from new position")
                        self.movement_start_y = None
                        # Re-acquire target from new position before retrying
                        self.pending_target.clear_history_for_reacquisition()
                        self.state = "REACQUIRING"
                        self.state_start_time = self.get_clock().now()
                    elif elapsed > self.final_retry_movement_timeout:
                        self.get_logger().warn(f"⚠️ Movement timeout after {elapsed:.1f}s - proceeding anyway")
                        self.movement_start_y = None
                        self.pending_target.clear_history_for_reacquisition()
                        self.state = "REACQUIRING"
                        self.state_start_time = self.get_clock().now()
    ...
    def _send_target_to_ur3(self):
        ...
        # Round to 3 decimal places to match target definitions
        pose.pose.position.x = round(rx, 3)
        pose.pose.position.y = round(ry, 3)
        pose.pose.position.z = round(rz, 3)
        pose.pose.orientation.w = 1.0
        self.target_pub.publish(pose)
        
        self.get_logger().info(f"📤 SENT #{track.id}: robot=[{round(rx, 3):.3f}, {round(ry, 3):.3f}, {round(rz, 3):.3f}]")

    def _publish_movement_command(self):
        """
        Publish a Twist command to move the swincar 0.05m in the -x-direction.
        This helps avoid singularities before the final beam attempt by changing
        the arm's approach angle from a different robot position.
        """
        twist = Twist()
        # Linear velocity: move negative x
        twist.linear.x = -0.5  # -0.5 m/s velocity (move in -x direction)
        twist.linear.y = 0.0
        twist.linear.z = 0.0
        twist.angular.x = 0.0
        twist.angular.y = 0.0
        ...
        self.get_logger().info(
            f"🚗 MOVING [-{self.final_retry_movement_x}m in x-direction] to avoid arm singularity on final retry"
        )
    ...
    def maybe_publish_targets(self, stamp, current_time: float):
        ...
        if is_retry and self.swincar_stopped:
            # Check if this is the FINAL retry attempt (last chance)
            is_final_retry = (best_track.beam_attempts >= best_track.max_beam_attempts)
            
            if is_final_retry:
                # Move the swincar 0.05m in x direction before final retry
                self.get_logger().warn(
                    f"⚠️ FINAL RETRY #{best_track_id} (attempt {best_track.beam_attempts}/{best_track.max_beam_attempts}) - moving first to avoid singularity"
                )
                self.movement_start_y = self.robot_y
                self._publish_movement_command()
                self.state = "MOVING_FOR_FINAL_RETRY"
                self.state_start_time = self.get_clock().now()
            else:
                # Normal retry (not final)
    ...
    def _publish_stop_signal(self, track: TrackedTarget, stamp):
        ...
        # Round to 3 decimal places to match target definitions
        point.point.x = round(rx, 3)
        point.point.y = round(ry, 3)
        point.point.z = round(rz, 3)
        self.target_point_pub.publish(point)
        
        self.get_logger().info(f"📡 STOP [{round(rx, 3):.3f}, {round(ry, 3):.3f}, {round(rz, 3):.3f}]")
    ...
    def publish_all_tracked(self, stamp):
        ...
        for track in self.tracked_targets.values():
            pose = Pose()
            # Round to 3 decimal places to match target definitions
            pose.position.x = round(track.world_x, 3)
            pose.position.y = round(track.world_y, 3)
            pose.position.z = round(track.world_z, 3)
