...
def generate_launch_description():
    ...
    default_world = os.path.join(str(Path.home()), "/home/alex/tese_ws/src/sim/worlds/t.sdf")
    ...
    def read_swincar_ur3_sdf():
        ...
        sdf = p.read_text()
        # Remove collision elements from swincar (keep only visual/inertial for rendering)
        # This matches collision tags for the swincar model specifically
        sdf = re.sub(r'\s*<collision>.*?</collision>\s*', '', sdf, flags=re.DOTALL)
    ...
    ld.add_action(Node(
        ...
        executable="beamastarstoped",
        name="beamastarstoped",
        output="screen",
        parameters=[
            robot_description,
            moveit_config.robot_description_semantic,
