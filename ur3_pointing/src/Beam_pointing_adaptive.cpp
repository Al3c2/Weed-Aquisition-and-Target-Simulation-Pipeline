...
// [NEW] Publishes actual trajectory duration to /arm_trajectory_duration
//       so the predictive detector can use real timing instead of fixed 5s
// [NEW] dwell_time parameter (default 0.15s) - holds beam on target after
//       execution to satisfy the 0.1s minimum beam-on-target requirement
// [NEW] Duration also published for joint-space fallback path

#include <rclcpp/rclcpp.hpp>
#include <geometry_msgs/msg/pose_stamped.hpp>
#include <geometry_msgs/msg/pose_array.hpp>
#include <std_msgs/msg/bool.hpp>
#include <std_msgs/msg/float32.hpp>          // [NEW] for trajectory duration publish
...
public:
    ...
    tf_listener_(*tf_buffer_)
  {
    ...
    // [NEW] Dwell time: how long to hold beam on target after execution
    // Must be >= 0.1s to satisfy mentor requirement. Default 0.15s for safety margin.
    dwell_time_ = declare_parameter<double>("dwell_time", 0.15);
    RCLCPP_INFO(get_logger(), "⏱️  Dwell time: %.2fs (requirement: >=0.10s)", dwell_time_);
    ...
    // [NEW] Trajectory duration publisher - lets the predictive detector
    // know the real arm movement time instead of using a fixed 5s guess
    traj_duration_pub_ = create_publisher<std_msgs::msg::Float32>("/arm_trajectory_duration", 10);
    RCLCPP_INFO(get_logger(), "📡 Publishing trajectory duration to /arm_trajectory_duration");
...
private:
  ...
  double dwell_time_;  // [NEW] beam dwell time after execution
  ...
  rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr traj_duration_pub_;  // [NEW]
  ...
  // [NEW] Helper: extract duration from a planned trajectory and publish it
  void publishTrajectoryDuration(const moveit::planning_interface::MoveGroupInterface::Plan& plan,
                                  const std::string& source)
  {
    const auto& points = plan.trajectory_.joint_trajectory.points;
    if (points.empty()) return;

    double duration = points.back().time_from_start.sec +
                      points.back().time_from_start.nanosec * 1e-9;

    RCLCPP_INFO(get_logger(), "   [%s] Trajectory duration: %.2fs", source.c_str(), duration);

    std_msgs::msg::Float32 msg;
    msg.data = static_cast<float>(duration);
    traj_duration_pub_->publish(msg);
  }
                 ...
                 bool check_validity = true)
  {
    ...
    // Note: removed isIKSolutionValid - it ran a full RRT plan which was rejecting
    // valid IK solutions because the swincar mesh blocked intermediate arm poses.
    // The actual planning step handles collision avoidance properly.
    return true;
  }
  ...
  // [CHANGE A] tryCartesianPath now calls publishTrajectoryDuration after IPTP
                        ...
                        moveit::planning_interface::MoveGroupInterface::Plan& plan)
  {
    ...
    if (!plan.trajectory_.joint_trajectory.points.empty()) {
      ...
      // [NEW] Publish the real Cartesian trajectory duration
      publishTrajectoryDuration(plan, "Cartesian");
    }
  ...
  // [CHANGE C] tryJointSpacePlan now also extracts and publishes trajectory duration
                         ...
                         moveit::planning_interface::MoveGroupInterface::Plan& plan)
  {
    ...
    // [NEW] Publish the real joint-space trajectory duration
    if (!plan.trajectory_.joint_trajectory.points.empty()) {
      publishTrajectoryDuration(plan, "JointSpace");
    }
  ...
  bool attemptTargetWithRecovery(const tf2::Vector3& target, int attempt_num = 1)
  {
    ...
    // Try Cartesian first, fall back to joint space
    ...
    // [CHANGE B] Dwell: hold beam on target for dwell_time_ seconds.
    // This satisfies the >=0.1s beam-on-target requirement.
    // Verification happens AFTER the dwell so we confirm beam stayed on target.
    RCLCPP_INFO(get_logger(), "   ⏱️  Holding beam for %.2fs...", dwell_time_);
    ...
    if (error < beam_error_threshold_) {
      RCLCPP_INFO(get_logger(), "✅ SUCCESS! Error: %.1fmm (dwell: %.2fs)", error * 1000, dwell_time_);
      return true;
    } else {
